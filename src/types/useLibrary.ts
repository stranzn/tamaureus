...
import { deleteFilesOnRemove } from "../store/settingsStore";
...
export function useLibrary() {
    ...
    try {
      ...
      const playlistsWithCovers = await Promise.all(
        playlistData.map(async (p) => {
          ...
          const thumbnails = [
            p.thumb1_base64
              ? { base64: p.thumb1_base64, mime: p.thumb1_mime }
              : null,
            p.thumb2_base64
              ? { base64: p.thumb2_base64, mime: p.thumb2_mime }
              : null,
            p.thumb3_base64
              ? { base64: p.thumb3_base64, mime: p.thumb3_mime }
              : null,
            p.thumb4_base64
              ? { base64: p.thumb4_base64, mime: p.thumb4_mime }
              : null,
  ...
  const handleDelete = async () => {
    ...
    try {
      await invoke("remove_track", { trackId: track.id }); // removes track from database

      // update ui right away — library removal already succeeded regardless
      // of whether the file delete below succeeds
      setTracks((prev) => prev.filter((t) => t.file_path !== track.file_path));

      if (deleteFilesOnRemove()) {
        await invoke("delete_track_file", { srcFile: track.file_path }); // deletes track from filesystem
      }
    } catch (err) {
  ...
  async function selectAudioFiles() {
    ...
    if (srcPath && typeof srcPath === "string") {
      ...
      try {
        const metadata = await invoke<any>("get_track_metadata", {
          path: srcPath,
        });
        setMeta({
